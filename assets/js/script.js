...
function startGreetingPreloader() {
    ...
    let index = 0;
    const swapDelay = isMobileViewport ? 145 : 170;
    const runDuration = isMobileViewport ? 2100 : 2600;
...
window.addEventListener('load', startGreetingPreloader);

document.addEventListener('visibilitychange',
    function () {
        if (document.visibilityState === "visible") {
            document.title = "Nirbhay's Portfolio";
...
function showProjects(projects) {
    ...
    const projectsToShow = projects.filter(project => selectedProjects.includes(project.name));
